...
class BaseWithCityHandler(BaseHandler):
    def prepare(self):
        if (self.get_secure_cookie('city_id') and self.get_secure_cookie('city_name') and self.get_secure_cookie('city_code'))or  self.request.uri.find("/select_city")>=0 or self.get_argument("city_id",None):
            pass
        else:
...
def require_basic_authentication(method):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        #
        # hasAuth = self.application.session_store.has_session(auth_decoded)
        #
        # if not hasAuth:
        #     pass
        ...
        #             if user.isactive > 0:
        #                 user.updatesignin()
        #                 self.application.session_store.set_session(auth_decoded, {}, None, expiry=24*60*60)
        #                 hasAuth = True
        #     except:
        #         pass
        #
