  ...
  .factory('User', function($resource, $q, $rootScope, $http, ShopCar, Favorite) {
    ...
    return {
        ...
        resource.save(_.extend({
          ...
          function(result) {
            if (result.flag === 1) {
              ...
              if (result.msg.portraiturl) {
                result.msg.portraiturl = config.domain + result.msg.portraiturl;
              }

              deferred.resolve(result);
              $rootScope.$broadcast('loginSuccess', result.msg);
      ...
      requestValidationCodeRegister: function(mobile) {
        ...
        $http.get(config.domain + '/mobile/vcode', {
            params: {
              mobile: mobile
            }
          })
          ...
          .catch(function(error) {
            deferred.reject({
              message: error.data.msg || '网络或服务器异常'
            });
          });
  ...
  .factory('Ad', function($resource, $http, $rootScope, $q) {
    ...
    return {
      ...
      fetch: function(cityId) {
        ...
        //  "href": "{\"value\": \"\", \"key\": \"1\"}",
        //  "id": 4,
        //  "img": config.domain + '/upload/ad/20151030143628.jpg',
        //  "title": "\u6ce8\u518c\u900110\u5143"
        // }]);
  ...
  .filter('ago', function() {
    return function(input) {
      var now = new Date().getTime(),
        diffValue = now - new Date(input).getTime(),
        result = '',
        minute = 1000 * 60,
        hour = minute * 60,
        day = hour * 24,
